...
### Gandercoin maintainers/release engineers, suggestion for writing release notes
...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in gandercoin, then:
...
### Build and sign Gandercoin Core for Linux, Windows, and macOS:

    export GITIAN_THREADS=2
    export GITIAN_MEMORY=3000
    
    pushd ./gitian-builder
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit gandercoin=v${VERSION} ../gandercoin/contrib/gitian-descriptors/gitian-linux.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit gandercoin=v${VERSION} ../gandercoin/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit gandercoin=v${VERSION} ../gandercoin/contrib/gitian-descriptors/gitian-osx.yml
...
Build output expected:
  ...
  4. macOS unsigned installer and dist tarball (`gandercoin-${VERSION}-osx-unsigned.dmg`, `gandercoin-${VERSION}-osx64.tar.gz`)
  5. Gitian signatures (in `gitian.sigs.ltc/${VERSION}-<linux|{win,osx}-unsigned>/(your Gitian key)/`)

### Verify other gitian builders signatures to your own. (Optional)

Add other gitian builders keys to your gpg keyring, and/or refresh keys: See `../gandercoin/contrib/gitian-keys/README.md`.
...
Codesigner only: Sign the macOS binary:
    ...
    transfer gandercoin-osx-unsigned.tar.gz to macOS for signing
    tar xf gandercoin-osx-unsigned.tar.gz
    ...
    $   xcrun altool --notarize-app --primary-bundle-id "org.gandercoin.Gandercoin-Qt" -u "<apple-id-email>" -p "@keychain:<apple-id-notarisation-app-specific-password>" --asc-provider <team-id-shortcode> -t osx -f gandercoin-${VERSION}-osx.dmg
    ...
    $   xcrun stapler staple dist/Gandercoin-Qt.app

Codesigner only: Sign the windows binaries:

    tar xf gandercoin-win-unsigned.tar.gz
...
Codesigner only: Commit the detached codesign payloads:
    ...
    cd ~/gandercoin-detached-sigs
    #checkout the appropriate branch for this release series
    rm -rf *
    tar xf signature-osx.tar.gz
    tar xf signature-win.tar.gz
    #copy the notarization ticket to detached-sigs repo
    cp dist/Gandercoin-Qt.app/Contents/CodeResources osx/dist/Gandercoin-Qt.app/Contents/
    git add -A
...
Create (and optionally verify) the signed Windows binaries:
    ...
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-win-signed --destination ../gitian.sigs/ ../gandercoin/contrib/gitian-descriptors/gitian-win-signer.yml
    ./bin/gverify -v -d ../gitian.sigs/ -r ${VERSION}-win-signed ../gandercoin/contrib/gitian-descriptors/gitian-win-signer.yml
...
gandercoin-${VERSION}-riscv64-linux-gnu.tar.gz

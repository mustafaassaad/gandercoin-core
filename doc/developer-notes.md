...
- [Development guidelines](#development-guidelines)
    - [General Gandercoin Core](#general-gandercoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Gandercoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play gandercoins" on the test network, if you
...
Gandercoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/gandercoin-project/gandercoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_gandercoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_gandercoin --log_level=test_suite
$ valgrind -v --leak-check=full src/gandercoind -printtoconsole
...
# A coverage report will now be accessible at `./test_gandercoin.coverage/index.html`.
```
...
To profile a running gandercoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep gandercoind` -- sleep 60
```
...
Gandercoin Core can be compiled with various "sanitizers" enabled, which add
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Gandercoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
file descriptors, it will cause problems with Gandercoin's `select()` loop, because
...
compatibility between nodes. This happened in Gandercoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Gandercoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `gandercoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `gandercoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Gandercoin Core, the wallet was always

...
const char * const BITCOIN_CONF_FILENAME = "gandercoin.conf";
...
#else
    const char* pszModule = "gandercoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gandercoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Gandercoin";
#else
    // Unix-like
    return pathRet / ".gandercoin";
#endif
#endif
}
